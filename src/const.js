import { countries } from './listCountries.js'
...
//     'Мы играем в игру угадай флаги. Я тебе отправляю массив стран. Твоя задача вытаскивать поочереди кажду страну из этого массива и прислать мне: 1) флаг этой страны и 2) четыре варианта ответа (один верный и три неверных). Затем ты ждешь моего ответа. Как только я отвечаю, ты проверяешь ответ и говоришь правильно ли ответил и затем отправляешь следующий флаг с вариантами ответа'
//     'Пришли мне эмоджи флага страны и четыре варинта название страны из этого региона. Одно верное название и три неверных. Верное название помести первым элементом в массиве. В таком виде {flag: 🇲🇺, countries: ["country1", "country2", "country3", "country4"]}'
//     'Пришли мне название этой страны и еще трех стран из этого же региона в таком формате {"countries": ["country1", "country2", "country3", "country4"]}'
...
export const errorResponse = {
    ...
    0: 'Произошла непредвиденная ошибка. Очистите контекст и попробуйте ещё раз.',
}
...
export const aiGuide = (counSteps, erros) => {
    return `В игре 'Угадай флаг' было задано ${counSteps} вопросов. Ошибки: ${erros}. Пришли кратку статистику по этой игре.`
}
